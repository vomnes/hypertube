// Package gplus implements the OAuth2 protocol for authenticating users through 42 API.
...
// New creates a new 42 API provider, and sets up important connection details.
...
// Provider is the implementation of `goth.Provider` for accessing 42 API.
...
// BeginAuth asks 42 API for an authentication end-point.
func (p *Provider) BeginAuth(state string) (goth.Session, error) { // state string
	url := "https://api.intra.42.fr/oauth/authorize?client_id=" + p.config.ClientID + "&redirect_uri=" + url.PathEscape(p.config.RedirectURL) + "&response_type=code"
	session := &Session{
		AuthURL: url,
	}
	return session, nil
...
// FetchUser will go to 42 API and access basic information about the user.
